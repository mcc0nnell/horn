...
Interactive analysis needs a different capability. A reader may want to inspect the same argument as topology, chronology, provenance, or dialogue frontier without editing the mural or pretending that those derived layouts are Horn's authored cartography.
...
The current projection set contains four views:
...
4. **Frontier** — a reader-facing projection derived from focus boxes through later responses to the terminal arguments in each mapped dialogue thread. It reverses semantic response edges for reading order without rewriting the canonical relations.
...
- Stable Horn node and citation IDs should be carried into projections so selections can synchronize across views without changing document identity.
- The Frontier projection must preserve the distinction between semantic response direction and reader-facing dialogue order defined in ADR 0007.
- The SVG mural remains the reference view for authored spatial grammar.

## Consequences

Horn can support multiple interactive ways of interrogating the same argument without becoming a generic graph viewer. ECharts can provide zooming, filtering, emphasis, data zoom, force-directed provenance inspection, frontier analysis, and coordinated views while the document kernel stays renderer-independent.
